...
/*    	for (int i = 0; i < c_cnt; i++){
    	...
    	int cur_c = -1;
        for (int i = 0; i < c_cnt; ++i)
        	if (cur_c == -1 || c_fac_len[i] < c_fac_len[cur_c]) cur_c = i;
        //cerr << "hello";                                                                     
...
//      cerr << "hello";
    	...
    	int cur_f = 0;
        for (auto facet : c_fac[cur_c]) {
        	if (f_ver[facet].size() > 0) {
        		cur_f = facet;
        		break;
        	}
        }	
    	int start_e = -1, start_v = -1;
    	//cerr << c_eds[cur_c].size() << endl;
        /*for (auto e: c_eds[cur_c])
        	cerr << e << " ";
        cerr << "\n";*/
    	for (int i = 0; i < c_eds[cur_c].size(); ++i) {
   // 		cerr << c_eds[cur_c][i] << " ";
    		pii &e = edges[c_eds[cur_c][i]];
    		if (!bad[c_eds[cur_c][i]] && (blocked[e.F] || blocked[e.S])) {
    			start_e = c_eds[cur_c][i];
...
//    		cerr << v << " ";
    	...
    	for (int i = 0; i < c_eds[cur_c].size(); ++i) {
    		pii &e = edges[c_eds[cur_c][i]];
    	    if (used[c_eds[cur_c][i]] || bad[c_eds[cur_c][i]]) continue;
    	    c_cnt++;
    	    c_edfs(c_eds[cur_c][i]);
    	    //cerr << "\n\n";
    	    c_fac[c_cnt - 1] = {cur_f};
      		c_fac_len[c_cnt - 1] = 1;
        }
        ...
        for (auto i : f_ver[cur_f]) {
    		if (ink1[i]) {
      			...
      			else f_ver[fk3].insert(f_ver[fk3].end(), b_path.begin(), b_path.end());
      			cur_color ^= 1;
    			continue;
    		}
    		if (cur_color) {
